...
import {
  STUDENT_COMMUNITY_POST_DRAFT_STORAGE_KEY,
...
function readDraftFromStorage(): CommunityPostDraft {
  ...
  try {
    const raw = sessionStorage.getItem(STUDENT_COMMUNITY_POST_DRAFT_STORAGE_KEY);
...
function writeDraftToStorage(draft: CommunityPostDraft) {
  sessionStorage.setItem(STUDENT_COMMUNITY_POST_DRAFT_STORAGE_KEY, JSON.stringify(draft));
}

export function StudentCommunityCreatePostView() {
  const t = useTranslations("student.dashboard.knowledgeCommunity.create");
