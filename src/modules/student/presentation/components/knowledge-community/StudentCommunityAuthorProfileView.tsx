...
import {
  useStudentCommunityAuthor,
  type CommunityAuthorTab,
} from "@/modules/student/application/hooks/useStudentCommunityAuthor";
...
import { StudentCommunityAuthorProfileSkeleton } from "@/modules/student/presentation/components/knowledge-community/StudentCommunityAuthorProfileSkeleton";
import { UserAvatarImageOrInitials } from "@/shared/presentation/components/user";
import { ApiFailureAlert } from "@/shared/presentation/components/ui/ApiFailureAlert";

export function StudentCommunityAuthorProfileView({ authorId }: { authorId: string }) {
  const t = useTranslations("student.dashboard.knowledgeCommunity.author");
  const tCommon = useTranslations("student.dashboard");
  const locale = useLocale();
  const routes = useScopedDashboardRoutes();
  const [tab, setTab] = useState<CommunityAuthorTab>("all");
  const { profile, articles, loading, error, setFollowing } = useStudentCommunityAuthor(authorId, tab);
  ...
  if (loading) {
    return <StudentCommunityAuthorProfileSkeleton label={tCommon("common.loading")} />;
  }
