...
import {
  getChatGroupsPage,
  getChatGroupsStatistics,
  type ChatGroupStatistics,
...
export type ChatGroupsFilterState = {
...
function mapStatusToApi(status: ChatGroupsFilterState["status"]): string | undefined {
  if (status === "all") return undefined;
  return status;
}

export function useChatGroupsDashboard(filters: ChatGroupsFilterState, pageNumber: number) {
  ...
  const statsQuery = useQuery({
    queryKey: ["admin-chat-groups-statistics", locale],
    queryFn: getChatGroupsStatistics,
  });
