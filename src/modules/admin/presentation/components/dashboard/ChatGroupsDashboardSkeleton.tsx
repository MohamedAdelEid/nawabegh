...
export function ChatGroupsDashboardSkeleton() {
  return (
    <div className="space-y-8" aria-hidden>
      <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div
            key={`chat-groups-stat-${index}`}
      ...
      >
        <div className="grid gap-4 xl:grid-cols-4">
          {Array.from({ length: 3 }).map((_, index) => (
            <Skeleton key={`chat-groups-filter-${index}`} className="h-11 w-full rounded-xl" />
          ))}
      ...
      >
        ...
        <div className="space-y-3 p-6">
          {Array.from({ length: 8 }).map((_, index) => (
            <Skeleton key={`chat-groups-row-${index}`} className="h-14 w-full rounded-xl" />
          ))}
        </div>
