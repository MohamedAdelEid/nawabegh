...
export function TeacherLiveAnalyticsDashboardSkeleton({
  label,
  withHeader = true,
}: {
  label?: string;
  ...
  return (
    <SkeletonScreen className="space-y-6" label={label}>
      {withHeader ? <DashboardHeaderSkeleton /> : null}
          ...
          <SkeletonCard className="rounded-[2rem]">
            <div className="space-y-4">
              ...
              {Array.from({ length: 3 }).map((_, index) => (
                <Skeleton key={`absent-${index}`} className="h-14 w-full rounded-xl" />
              ))}
            </div>
          </SkeletonCard>
        </div>
